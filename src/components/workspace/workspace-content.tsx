...
export function WorkspaceContent({ workspaceId }: WorkspaceContentProps) {
  ...
  const selectedFolderFiles = selectedFolderId
    ? files.filter(
        (file) => !file.inTrash && file.folderId === selectedFolderId
      )
    : []
            ...
            <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
              {nonTrashedFolders.map((folder) => (
                <div
                  ...
                  className={cn(
                    'group relative text-xl flex items-center gap-3 rounded-lg border p-4 hover:bg-accent hover:shadow-md transition-all cursor-pointer',
                    selectedFolderId === folder.id && 'bg-accent border-primary'
                  )}
                >
                  <Folder className='size-6 shrink-0 text-blue-500' />
          ...
          <div>
            <div className='mb-4 flex items-center justify-between'>
              <h2 className='text-xl font-semibold text-muted-foreground'>
                {selectedFolderId
                  ? `Files in ${
                      nonTrashedFolders.find((f) => f.id === selectedFolderId)
                        ?.title || 'Folder'
                    }`
                  : 'Files'}
              </h2>
              ...
              <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
                {nonTrashedFiles.map((file) => (
                  <div
                    ...
                    className='group text-xl relative flex items-center gap-3 rounded-lg border p-4 hover:bg-accent hover:shadow-md transition-all cursor-pointer'
                  >
                    {file?.iconId ? (
                      <div className='text-2xl'>{file.iconId}</div>
                    ) : (
